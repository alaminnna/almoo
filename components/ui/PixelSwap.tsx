...
const coverScale = (
  ...
  const corner =
    Math.SQRT1_2 / (Math.SQRT2 * (0.5 - p) + p);
...
const buildGrid = ({
  ...
  if (columns * rows > MAX_PIXELS) {
    size = Math.ceil(
      size * Math.sqrt((columns * rows) / MAX_PIXELS)
    );
    columns = Math.max(
      1,
      Math.ceil((width + gap) / (size + gap))
...
function PixelSwap({
  ...
  const [shownActive, setShownActive] = useState(
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach((pixel, index) => {
      ...
      const originX =
        pixel.left + frozenGrid.size / 2;
      const originY =
        pixel.top + frozenGrid.size / 2;
  ...
  const renderLayer = (
    ...
    return (
      <div
        ...
        data-visible={
          isShown &&
          !(transition && index === incomingIndex)
        }
